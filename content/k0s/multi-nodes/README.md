...
In this part, make sure to run the commands on node1.
...
```
sudo k0s install worker --token-file /tmp/worker_token
```
...
node2   Ready    <none>   32s   v1.21.3+k0s
```
...
Remove k0s from node1 and node2 running the following commands on both VMs:
